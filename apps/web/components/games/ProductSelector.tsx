...
const schema = z.object({
  gameUid: z.string().min(1, 'Game UID is required').max(100),
...
export function ProductSelector({ products }: Props) {
      ...
      zodResolver(
        schema.superRefine((data, ctx) => {
          ...
          if (p?.requiresServer && !data.gameServer?.trim()) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['gameServer'], message: 'Server is required for this product' });
          }
          if (p?.requiresUsername && !data.gameUsername?.trim()) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['gameUsername'], message: 'Username is required for this product' });
          }
        }),
      ),
    [],
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (!product) {
      setError('root', { message: 'Pick a package first' });
      return;
    }
    try {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8">
      <section>
        <h2 className="font-display text-xl text-frost mb-4">1 · Pick a package</h2>
        {products.length === 0 && (
          <p className="text-frost/40 py-8">No packages available for this game yet.</p>
        )}
      ...
      <section>
        <h2 className="font-display text-xl text-frost mb-4">2 · Enter your details</h2>
        <form
          onSubmit={handleSubmit(onSubmit)}
          className="pixel-cut bg-panel border border-frost/10 p-6 space-y-4"
        >
          <div>
            <label htmlFor="gameUid" className={labelClasses}>Game UID</label>
            <input id="gameUid" {...register('gameUid')} className={inputClasses} placeholder="Your in-game player ID" />
            {errors.gameUid && <p className="text-pink text-xs mt-1">{errors.gameUid.message}</p>}
          </div>

          {selected?.requiresServer && (
            <div>
              <label htmlFor="gameServer" className={labelClasses}>Server</label>
              <input id="gameServer" {...register('gameServer')} className={inputClasses} placeholder="e.g. Asia" />
          ...
          {selected?.requiresUsername && (
            <div>
              <label htmlFor="gameUsername" className={labelClasses}>Username</label>
              <input id="gameUsername" {...register('gameUsername')} className={inputClasses} placeholder="Your in-game name" />
              {errors.gameUsername && <p className="text-pink text-xs mt-1">{errors.gameUsername.message}</p>}
            </div>
          )}

          <div>
            <label htmlFor="couponCode" className={labelClasses}>Coupon code (optional)</label>
          ...
          >
            {isSubmitting
              ? 'Placing order…'
              : selected
                ? `Pay ฿${Number(selected.priceSell).toLocaleString('th-TH')} from credits`
                : 'Pick a package to continue'}
          </button>
          <p className="text-xs text-frost/40 text-center">
            Paid from your PixelPay credits. Short on credits? Top up on the wallet page.
          </p>
        </form>
      </section>
    </div>
